...
pytest -v
```
All 20 tests assert cryptographic, database, and rate-limiting controls.

### 2. Execute the Interactive Demo
```bash
python3 demo.py
```
Walks through 10 scenario paths—contrasting vulnerable and hardened implementations of legitimate requests and adversarial attacks (such as BOLA, forged tokens, sync racing, and exfiltration attempts).
...
| ├── [`auth.py`](cloud_security_case/auth.py) | Cognito-simulating asymmetric JWT token issuer & verifier. |
| ├── [`authorization.py`](cloud_security_case/authorization.py) | Insecure vs secure query fetchers enforcing tenant isolation. |
...
| ├── [`audit.py`](cloud_security_case/audit.py) | Structured audit logging with recursive PII/JWT redaction. |
| ├── [`detection.py`](cloud_security_case/detection.py) | Anomaly engine monitoring access rates and auth failures. |
